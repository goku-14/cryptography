...
void gt_multiplication_test() {
    ...
    // 设置标量（大数）
    cinstr(scalar, (char*)"1234567890ABCDEF1234567890ABCDEF1234567890ABCDEF1234567890ABCDEF");
    ...
    // 在GT群中进行标量乘法运算: scalar * z (等价于 z^scalar)
    result = pfc.power(z, scalar);
    
    auto end = std::chrono::high_resolution_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
    execution_time = elapsed.count() * 1e-6;
    
    cout << "Multiplication in GT run time:" << execution_time << " ms" << endl;
