                 ...
                 const AES_KEY *key)
{
    ...
    s0 =
        ...
        ((unsigned int)Td4[(t3 >> 16) & 0xff] << 16) ^
        ((unsigned int)Td4[(t2 >>  8) & 0xff] <<  8) ^
        ((unsigned int)Td4[(t1      ) & 0xff])       ^
        rk[0];
    PUTU32(out     , s0);
    s1 =
        ((unsigned int)Td4[(t1 >> 24)       ] << 24) ^
        ((unsigned int)Td4[(t0 >> 16) & 0xff] << 16) ^
        ((unsigned int)Td4[(t3 >>  8) & 0xff] <<  8) ^
        ((unsigned int)Td4[(t2      ) & 0xff])       ^
        rk[1];
    PUTU32(out +  4, s1);
    s2 =
        ((unsigned int)Td4[(t2 >> 24)       ] << 24) ^
        ((unsigned int)Td4[(t1 >> 16) & 0xff] << 16) ^
        ((unsigned int)Td4[(t0 >>  8) & 0xff] <<  8) ^
        ((unsigned int)Td4[(t3      ) & 0xff])       ^
        rk[2];
    PUTU32(out +  8, s2);
    s3 =
        ((unsigned int)Td4[(t3 >> 24)       ] << 24) ^
        ((unsigned int)Td4[(t2 >> 16) & 0xff] << 16) ^
        ((unsigned int)Td4[(t1 >>  8) & 0xff] <<  8) ^
        ((unsigned int)Td4[(t0      ) & 0xff])       ^
        rk[3];
    PUTU32(out + 12, s3);
}
...
int AES_ecb_encrypt(unsigned char *out, int *out_len, const unsigned char *in, int in_len, const AES_KEY *key)
{
    ...
    if(in_len > 0)
        memcpy(buf, in + i, in_len);

    if (pkcs5_pad(buf, in_len, AES_BLOCK_SIZE) == -1)
        return -1;
...
int AES_ecb_decrypt(unsigned char *out, int *out_len, const unsigned char *in, int in_len, const AES_KEY *key)
{
    ...
    if(in_len % AES_BLOCK_SIZE != 0 || in_len <= 0)
        return -1;
    ...
    if (pkcs5_unpad(&unpadded_len, out + i, AES_BLOCK_SIZE) == -1)
        return -1;

    if(out_len != NULL) 
        *out_len = i + unpadded_len;
...
int AES_ctr_encrypt(unsigned char *out, int *out_len, const unsigned char *in, int in_len, const AES_KEY *key)
{
    int i, j, n, tail;
    unsigned int lo, hi;
    unsigned int lo_be, hi_be;
    unsigned char ctr[16];
    unsigned char x[16];

    n = (in_len + AES_BLOCK_SIZE - 1) / AES_BLOCK_SIZE;
    tail = in_len % AES_BLOCK_SIZE;
    lo = 0, hi = 0;
    memset(ctr, 0, 16);

    for (i = 0; i < n; i++) {
        AES_encrypt(ctr, x, key);
        if (i == (n - 1)) {
            for (j = 0; j < tail; j++) {
                out[j] = in[j] ^ x[j];
            }
        ...
        lo_be = to_be32(lo);
        hi_be = to_be32(hi);
        memcpy(ctr + 8, &hi_be, 4);
        memcpy(ctr + 12, &lo_be, 4);
        in += AES_BLOCK_SIZE;
        out += AES_BLOCK_SIZE;
    }
