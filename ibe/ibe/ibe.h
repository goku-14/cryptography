...
/* 接收方使用接收方私钥(usr_privkey)验证发送方(usr_id)计算消息认证码 */
...
/* 使用接收方id和发送方私钥生成对称加密密钥加密明文(in) */
EXPORT_API int mac_encrypt(unsigned char *out, int *out_len, const unsigned char *in, int in_len, 
                           unsigned int usr_id, unsigned char *usr_privkey);

/* 使用发送方id和接收方私钥生成对称加密密钥解密密文(in) */
EXPORT_API int mac_decrypt(unsigned char *out, int *out_len, const unsigned char *in, int in_len, 
                           unsigned int usr_id, unsigned char *usr_privkey);
